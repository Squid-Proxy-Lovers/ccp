...
Agents get read, search, creation, append, and temporary team-status operations. Destructive operations and server management are CLI-only.
...
An enrollment owns its status records. Named agents sharing one enrollment can manage each other's records, so use separate enrollments when workers should not share that trust boundary.
